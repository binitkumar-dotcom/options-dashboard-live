...
if not deep_dive_ticker:
    ...
    try:
        ...
        if stock_data.empty:
            ...
            # *** FIX: Clean data BEFORE trying to access it ***
            stock_data.dropna(inplace=True)
            
            if stock_data.empty:
                st.error(f"No valid trading data found for {deep_dive_ticker} after cleaning.")
            else:
                # *** FIX: This line is now safe ***
        ...
        st.error(f"A critical error occurred while fetching HISTORICAL data: {e}")