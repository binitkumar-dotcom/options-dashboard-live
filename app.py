...
if ticker_list:
    ...
    # *** CHANGE: Fixed 'use_container_width' warning ***
...
if not deep_dive_ticker:
    ...
    try:
        # *** CHANGE: Added auto_adjust=True to fix yfinance warning ***
        stock_data = yf.download(deep_dive_ticker, period="1y", progress=False, auto_adjust=True)
        
        if stock_data.empty:
            st.error(f"Could not download historical data for {deep_dive_ticker}. Check symbol or yfinance status.")
        else:
            ...
            # *** CHANGE: Fixed 'use_container_width' warning ***
            st.plotly_chart(fig, use_container_width=True) # Plotly still needs this param
            ...
            with col1:
                ...
                # *** CHANGE: Fixed 'use_container_width' warning ***
                st.plotly_chart(fig_rsi, use_container_width=True) # Plotly still needs this param
            with col2:
                fig_macd = go.Figure()
                ...
                # *** CHANGE: Fixed 'use_container_width' warning ***
                st.plotly_chart(fig_macd, use_container_width=True) # Plotly still needs this param
            ...
            # *** CHANGE: Added detailed try/except to find the exact error ***
            try:
                stock_yft = yf.Ticker(deep_dive_ticker)
                
                # This is the most common failure point
                st.write(f"Fetching options expiration dates for {deep_dive_ticker}...")
                exp_dates = stock_yft.options
                st.write(f"Found dates: {exp_dates}")
                
                if not exp_dates:
                    st.error(f"No option expiration dates found for {deep_dive_ticker}. yfinance may not support options for this ticker or may be temporarily down.")
                else:
                    ...
                    def get_greeks(ticker, expiry):
                        st.write(f"Calculating Greeks for {expiry}...")
                        ...
                        st.write("Greeks calculation complete.")
                        return calls, puts
                ...
                st.error("This is common with yfinance for NSE options.")
